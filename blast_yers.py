...
tags = ["locusID"] # tags in fasta header blast hit for which the information needs to be transferred to an entry field, add additional tags seperated by a , and in double quotes
fields = ["CoreEnterolitica"] # fields in the database to contain the info in the tags, define in the same order as the tags, add additional fields seperated by a , and in double quotes
BlastDbname = "CoreEnterolitica" # name of the blast database to be used for the blast analysis
exp = "de novo" # name of experiment type to use for the blast analysis
identity_threshold = 75 #Only if the first hit has an identity above the threshold, the results of this hit are parsed
coverage_threshold = 90 #Only if the alignment length/query length of the first hit is higher than the threshold, the results of the hit are parsed 
CommentField = "Comments_blast" #Name of field to contain comments on the blast results
IDField = "Identity_blast" #Name of the field to store the identity with the hit
